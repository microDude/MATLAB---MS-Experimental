function [fXsolMLSP,fXsolMLSP12norm,fobjFuncMLSP,fconstrainMLSP,fgamma] = fMLSP_revB(fA,fArowsum,fY,fXsolInitial,fepsilonMLSP,fNiterMLSP,fNiterGamma,fgammaMin,fgammaMax,fepsilonThreshold)
...
%         disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
        ...
        disp(strcat('current Gamma Ranging step = ',num2str(fstepCntr)));
%         disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
...
%         disp(strcat('current fGammaMin = ',num2str(fgammaMin)))
        ...
        for fiter = 1:fNiterMLSP
            ...
            % one way to fix this problem is to use the same update formula
            % but with hlp4/hlp3 terms, but there occurs anither problem
            % smth/zero. To fix it, calculate fHlp4'./fHlp3 only on
            % positions where fHlp3~=0
            % positions where fHlp3~=0
            fHlp11 = find(fHlp3 ~= 0);
            ...
            % fHlp12 contains fHlp4'./fHlp3, but is updated only when denominator is not zero
            fHlp12 = zeros(fN,fK);
            fHlp12(fHlp11) = fHlp14(fHlp11)./(-fHlp3(fHlp11));
            % fHlp13 contains fHlp4'./(fHlp3.^2), but is updated only when denominator is not zero
            fHlp13 = zeros(fN,fK);
            fHlp18 = fHlp3.^2;
            % indices where denominator is not zero
            fHlp20 = find(fHlp18 ~=0 );
            fHlp13(fHlp20) = fHlp14(fHlp20)./(fHlp18(fHlp20));
            
            % check if we ever divide smth nonzero by zero
            fHlp21 = ismember(fHlp15,fHlp20);
            fhlp22 = (fHlp21 == 0);
            % if we divide by zero output problem and stop
            if sum(fhlp22)>0
                disp('   ')
            ...
            fXsolMLSP = ((0.5*fHlp12).*(1./(1+sqrt(1+fHlp13))))';
...
%             disp('constr for max gamma < eps');
        ...
        for fiter = 1:fNiterMLSP
            ...
            % update the solution matrix
            fXsolMLSPmin = ((0.5*fHlp12).*(1./(1+sqrt(1+fHlp13))))';
...
%         title('objective function for current Lagrangain-gamma');
%         grid on
...
%         disp(strcat('gammaMAXwas =  ',num2str(fgammaMax)));
        if fconstrainMLSP(fiGamma) > fepsilonMLSP
            ...
            if mod(fiGamma,5) == 0
                ...
                disp(strcat('gammaMINnow =  ',num2str(fgammaMin)));
                disp(strcat('gammaMAXwnow =  ',num2str(fgammaMax)));
                disp('   ');
            end
        else
            % then decrease gamma
