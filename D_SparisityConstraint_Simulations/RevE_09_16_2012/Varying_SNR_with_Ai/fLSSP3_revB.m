function [fXsolLSSP,fXsolLSSP12norm,fobjFuncLSSP,fconstrainLSSP,fdelta] = fLSSP3_revB(fA,fY,fXsolInitial,fepsilonLSSP,fNiterE,fNiterDelta,fdeltaMin,fdeltaMax,fepsilonThreshold)
...
fstepCntrMax = 40;
% multiples of delta to increase delta or to decrease it (for the part of
% the code where we are serching the range for delta)
fdeltaIncreaseFactor = 1e1;
fdeltaDecreaseFactor = 1e-1;
%~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
...
%         disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
        ...
        %disp(strcat('current Delta step = ',num2str(fstepCntr)));
%         disp('~~~~~~~~~~~~~~~~~~~~~~~~~~~~');
...
%         disp(strcat('current fDeltaMin = ',num2str(fdeltaMin)))
        ...
        fdeltaHlp = fdeltaMax;
        % initializa external objective
        fobjExternalHlp = zeros(1,fNiterE);
        ...
        for fiterE = 1:fNiterE
            ...
            while  fInternalLoopExit == 0 && fInternalIterCntr<fNiterIMax
                ...
                for fiK = 1:fK
                    ...
                    fobjInternal(fInternalIterCntr) = fobjInternal(fInternalIterCntr) + fdeltaHlp*((fXsolLSSPin(fiK,:)-fXsolLSSP(fiK,:))*fM{fiK}*(fXsolLSSPin(fiK,:)-fXsolLSSP(fiK,:))'-2*(fXsolLSSPin(fiK,:)-fXsolLSSP(fiK,:))*fV(:,fiK)) + norm(fXsolLSSPin(fiK,:),2);
                end;
                fobjInternal(fInternalIterCntr) = fobjInternal(fInternalIterCntr) + fdeltaHlp*fC;
                ...
                fobjExternalHlp(fiterE) = sum(sqrt(sum(fXsolLSSPin.*fXsolLSSPin,2)))+ fdeltaHlp*(fconstrainLSSPhlp - fepsilonLSSP);
                % compare fobjFuncLSSPhlp with the objective on the last
                % external iteration. NB!!! objective function should
                % decrease!!!
                if fiterE > 1
                    if fobjExternalHlp(fiterE-1) > fobjExternalHlp(fiterE)
            ...
            %             % delta and current external iteration
            %             figure(999)
            %             plot(fobjInternal);
            %             grid on
        ...
        %         % for degugging external objective function
        %         figure(333)
        %         plot(fobjExternalHlp);
        %         grid on
...
%         disp(strcat('epsilonLSSP  = ',num2str(fepsilonLSSP)))
%         
        %~~~~~~~~~~~~~~~~~~~~~~% MIN %~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
        ...
        if fconstrDeltaMaxFlag && fconstrDeltaMinFlag
            ...
            if fconstrDeltaMaxFlag == 0
                ...
                fdeltaMax = fdeltaMax*fdeltaIncreaseFactor;
            end;
            ...
            if fconstrDeltaMinFlag == 0
                ...
                fdeltaMin = fdeltaMin*fdeltaDecreaseFactor;
            end;
        end;
    end; % while
...
%         disp(strcat('epsilonLSSP =  ',num2str(fepsilonLSSP)));
%         disp(strcat('constraint for current gamma =  ',num2str(fconstrainLSSP(fiDelta))));
%         disp(strcat('current delta =  ',num2str(fdelta(fiDelta))));
%         disp(strcat('deltaMINwas =  ',num2str(fdeltaMin)));
%         disp(strcat('deltaMAXwas =  ',num2str(fdeltaMax)));
