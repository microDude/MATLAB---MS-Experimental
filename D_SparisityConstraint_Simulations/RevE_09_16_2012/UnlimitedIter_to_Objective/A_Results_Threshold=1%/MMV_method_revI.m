...
m = 30;                                     % Length of a_i, x_i, y_i, i.e. Length of each spectra
...
alpha = 2:0.01:3;                           % Sample space used to generate 'a' and likewise A.

% Grid Search Parameters
n = [10];                         % Column space of X and y, i.e. number of captured spectra
delta = 1:1:10;                   % Card(x) pattern spacing
SNR = -10:1:30;             % SNR (dB) of the signal

% Define the solution space
XSolGrid  = cell(length(n),length(delta),length(SNR));
XTrueGrid = cell(length(n),length(delta),length(SNR));
numIterRequired = cell(length(n),length(delta),length(SNR));
...
for iN=1:length(n)
    ...
    for iDELTA=1:length(delta)
        ...
        for iSNR=1:length(SNR)
            ...
            X([2,2+delta(iDELTA)],:) = minAMPx + (minAMPx.*rand(Cardx,n(iN))); %Uniform random values assigned to fixed x values
            ...
            numIterRequired{iN,iDELTA,iSNR} = TotalNumIter;
     
        end
    end
    save(['MLSP_revC_n=',num2str(n(iN)),'_SNR=',num2str(SNR(iSNR)),'_Delta=',num2str(delta(iDELTA)),'_TotalIter=',num2str(TotalNumIter)]);
