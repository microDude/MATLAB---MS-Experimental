...
iter.statistical = 10;                       % Number of Statistical Expirements
iter.final = 200;                           % Number of regression iterations for the final solution
LagrangianHunting.numSteps = 10;            % Number of steps allowed to hone in on the true Lagrangain
LagrangianHunting.iter = 1000;               % Number of regression iterations per step for lagrangian Hunting
LagrangianHunting.min = 1e-6;               % Starting Lagrangian min
...
alpha = 2:0.01:8;                           % Sample space used to generate 'a' and likewise A.

% Grid Search Parameters
n = [20,30,40];                         % Column space of X and y, i.e. number of captured spectra
delta = [1:1:10];                   % Card(x) pattern spacing
SNR = [-10,-5,0,5,10,15,20];             % SNR (dB) of the signal

% Define the solution space
XSolGrid  = cell(length(n),length(delta),length(SNR),iter.statistical);
XTrueGrid = cell(length(n),length(delta),length(SNR));
numIterRequired = cell(length(n),length(delta),length(SNR),iter.statistical);
...
for iN=1:length(n)
    ...
    for iDELTA=1:length(delta)
        ...
        for iSNR=1:length(SNR)
            ...
            minAMPx = 10^(SNR(iSNR)/10); % Figure out the min for the x amplitude
            X = zeros(m,n(iN));
            ...
            X([2,2+delta(iDELTA)],:) = minAMPx + 0.5*minAMPx.*ones(Cardx,n(iN)); % Same xAmplitude assigned to each value 
            ...
            for iSTATISTICAL=1:iter.statistical
                ...
                switch solver.type
                    ...
                    case 3
                        ...
                        [Xsol,~,~,constraint,lagrangian,TotalNumIter] = fMLSP_revC(XTrueGrid{iN,iDELTA,iSNR},A,fArowsum,y,(1+(5-1).*rand(m,n(iN))),epsilon,LagrangianHunting.iter,...
                ...
                x_est(x_est < (max(x_est)*1e-2)) = 0;    % Zero-out any element less then 3-order of magnitudes of the peak
                
                % Store the result
                display(['Time to complete one solution : ',num2str(toc(timeiSTAT)),' secs']);
                XSolGrid{iN,iDELTA,iSNR,iSTATISTICAL} = x_est;
                numIterRequired{iN,iDELTA,iSNR,iSTATISTICAL} = TotalNumIter;
            end
        end
        save(['TempSave','_n=',num2str(n(iN)),'_Delta=',num2str(delta(iDELTA))]);
    end
