...
langrangian.max = 0.01;     % Starting Lagrangian max
...
for R = 1:Riter
    ...
    for t=1:n
        ...
        A{1,t} = toeplitz(zeros(1,m),a)';         % Cell array which holds A_i
    end
    ...
    for iterL = 1:langrangian.niter
        ...
        %else
            Xsol = fSolveForX_revC(A,y,lagrangian(iterL),niter,(1+(5-1).*rand(m,n)),0);
        %end
